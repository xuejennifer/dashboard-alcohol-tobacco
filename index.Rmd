...
- Since there are clear divisions of rural and urban areas within the U.S., which also follow closely to political identification, we predict the more rural areas of the country (South and Midwest) will drink and smoke more than the rest of the country, possibly due to conservative/traditional values and the lack of safer legal substance alternatives.
...
#### **While visualizations are informative, we sought to corroborate our findings with statistical analyses. For the full regression outputs, refer to the Appendix.**
...
### Regressions: Tobacco (Current Smokers)
...
alc_tobacco_data %>% mutate(smoking = (smoke_everyday + smoke_some)*100) %>%
  ...
  scale_x_log10() +
  labs(x = "Gallons per Capita", y = "% Smokers", 
...
First, we visualize the consumption of both substances across states. The x-axis captures alcohol consumption, in gallons per capita, and the y-axis captures the percentage of smokers. This dynamic bubble plot captures change in consumption from 1995-2010. 
...
- *Across all alcohol types, Utah and West Virginia have the lowest overall alcohol consumption rates whereas New Hampshire and Nevada have the highest rates. In terms of tobacco, Utah consumes the least while Kentucky and West Virginia have the highest average consumption, over time. Nevada is also a relatively high consumer while New Hampshire ranks towards the middle on tobacco consumption. The third hypothesis is supported.*
...
- *As per the output of the decision tree, we do observe, on average, mild substitution effects between tobacco and alcohol for some states.*
...
### Tobacco Regression - Never Smoked
