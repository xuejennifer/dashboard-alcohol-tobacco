...
American healthcare institutions like the National Institutes of Health (NIH) and the Centers for Disease Control and Prevention (CDC) have meticulous datasets for alcohol and tobacco consumption in the United States. Yet, there are few analyses that look at the consumption of these substances side-by-side over time. In this project, we are interested in analyzing the longitudinal effects and patterns of alcohol and tobacco consumption in the U.S. at national, regional, and state levels. 
...
- Since there are clear divisions of rural and urban areas within the U.S., which also follow closely to political identification, we predict the more rural areas of the country (South and Midwest) will drink and smoke more than the rest of the country, possibly due to conservative/traditional values and the lack of other, safer, legal substance alternatives.

**4) Do there seem to be any substitutions between alcohol and tobacco?**

- It is possible that the effectiveness of smoking campaigns results in smoking rates declining for certain states; thus, we hypothesize that, in turn, alcohol consumption rates and smoking rates might covary. 
...
- Scatterplots, to visualize differences across states
...
alc2 %>% group_by(year, per_capita_ID) %>% 
  ...
  labs(title ="Average Alcohol Consumption, 1977-2018", 
...
alc2 %>% group_by(state) %>% 
  ...
  ggplot(aes(x=state, y=avg_drinks)) + geom_point(shape=18) +
    theme_bw() + ylim(0, 2.5) +
...
alc2 %>% group_by(state) %>%
  ...
  arrange(avg_beers) %>%
  ggplot(aes(x=state, y=avg_beers)) + geom_point(shape=18) +
    theme_bw() + ylim(0, 2.5) +
    theme(axis.text.x = element_text(angle = 90)) +
...
alc2 %>% group_by(state) %>%
  ...
  arrange(avg_wine) %>% 
  ggplot(aes(x=state, y=avg_wine)) + geom_point(shape=18) +
    theme_bw() + ylim(0, 2.5) +
    theme(axis.text.x = element_text(angle = 90)) +
...
alc2 %>% group_by(state) %>%
  ...
  arrange(avg_spirits) %>% 
  ggplot(aes(x=state, y=avg_spirits)) + geom_point(shape=18) +
    theme_bw() + ylim(0, 2.5) +
    theme(axis.text.x = element_text(angle = 90)) +
...
tobacco_data %>% mutate(smoking = `Smoke everyday` + `Smoke some days`) %>% 
  ...
  group_by(State) %>% summarize(smoking = mean(smoking)*100) %>%
  arrange(smoking) %>% 
  mutate(state = fct_reorder(State, smoking)) %>%
  ggplot(aes(x=state, y=smoking)) + geom_point() +
    theme_bw() + ylim(0,35)+
    theme(axis.text.x = element_text(angle = 90)) +
...
#### **While visualizations are informative, we sought to corroborate our findings with statistical analyses. For the full regression outputs, please see the Appendix.**
...
ALLALC %>% ggplot(aes(x=xvals, y=yvals)) + 
  geom_point(shape=18) + geom_errorbar(aes(ymin=lower, ymax=upper), alpha=.3) + 
  theme_bw() +
...
Our first regression (R$^2$ = .902) uses State and Year to predict the current smokers' percentage. Year is set as a factor, so we can understand year-to-year changes. The intercept, which captures the year 1995 and the state of Alabama, tells us that the estimated percentage of smokers is 24.6%. While smoking habits *rose from 1996 to 2003*, the rate at which smoking increased declined until 2003, where smoking rates declined. Smoking habits have continued to decline through 2010, where the rate was down 4.5% from the intercept. **Every year from 2004 to 2010 showed statistically significant declines in smoking rates.** Analyzing the state portions of the first regression, we see that most states have negative values concerning the intercept, showing that those states have lower rates of current smokers. A majority of which are statistically significant. The state of *Utah* stands out the most, with a *-11.8* estimate, predicting that people in Utah only smoke at half the rate of the regression's intercept. Conversely, *Kentucky* has the highest rate of current smokers, sitting at a statistically significant *+5.3* above the intercept. 
...
#### **In this section, we examine the relationship and tradeoff between alcohol and tobacco more closely.**


Column
-----------------------------------------------------------------------

### Dynamic Bubble Plot: Alcohol and Tobacco

```{r}
alc_tobacco_data %>% mutate(smoking = (smoke_everyday + smoke_some)*100) %>%
  ggplot(aes(x = ethanol_all_drinks_gallons_per_capita, y=smoking, 
             size = population, colour = Region, label=state)) +
  geom_point(aes(size = population, frame = year, ids = state), show.legend = FALSE) +
  scale_color_viridis_d(guide = FALSE) +
  scale_x_log10() +
  labs(x = "Alcohol Consumption in Gallons per Capita", y = "% Smokers", 
       title = "Alcohol and Tobacco Consumption",
       subtitle = "1995-2010") +
...
### Dynamic Bubble Plot Interpretation

First, we visualize the consumption of both substances across states. The x-axis captures alcohol consumption, in gallons per capita, and the y-axis captures the percentage of smokers. This dynamic bubble graph captures change in consumption from 1995-2010. 

The states are grouped into four regions: Midwest (purple), Northeast (blue), South (green), and West (yellow).
...
names(alc_tab_tr)[10] <- "All drinks (gallons per capita)"
...
#decision tree 
decisiontree <- rpart(formula = smoke_status ~ `All drinks (gallons per capita)`, 
                      data = other_train,
                      method  = "class")
...
To corroborate the insights that we visually gleaned, we aimed to determine whether there were any substitutions of one substance for the other across states, over time. This leads us to the crux of our report: a decision tree that aims to predict alcohol and tobacco use and the inter-relations between the two substances.
...
*Note.* While not shown in this present report, accuracy was unsurprisingly, slighty higher at 84% with train data.
...
- *We find that alcohol consumption dips between 1995 and 2000, but then indeed increases afterward, largely fueled by greater consumption per capita of wine and spirits. While beer remains the most consumed out of the three choices, its consumption per gallon is decreasing relative to the other two alcohol selections.*
...
- *Across all alcohol types, Utah and West Virginia have the lowest overall alcohol consumption rates whereas New Hampshire and Nevada have the highest rates. In terms of tobacco, Utah consumes the least while Kentucky and West Virginia have the highest average consumption, over time. Nevada is still a relatively high consumer and New Hampshire is in the middle. The third hypothesis is supported.*
...
* Walid Hedidar: interactive sliding graphs, classifications, alcohol graph generation
...
* Jon Sirota: dashboard creation, alcohol and tobacco dynamic bubble plot, alcohol visualizations
* Cory Winkler: tobacco statistical analyses, tobacco regression interpretations 
* Jen Xue: alcohol and tobacco dynamic bubble plot, tobacco graph generation, project management
